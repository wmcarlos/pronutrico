...
require_once('../controladores/corTransporte.php');
$objFunciones = new clsFunciones;
$operacion = $lcOperacion;
$listo = $lcListo;
if(($operacion!='buscar' && $listo!=1) || ($operacion!='buscar' && $listo==1))
{
$id = $objFunciones->ultimo_id_plus1('transporte','codigo');
}
?>
...
<td><input type='text' disabled='disabled' maxlength='' name='txtdireccion' value='<?php print($lcDireccion);?>' id='txtdireccion' class='validate[required]'/></td>
</tr>

<input type='hidden' name='txtoperacion' value='des'>
<input type='hidden' name='txtvar_tem' value='<?php print($lcCodigo); ?>'>
</table>
<?php $objFunciones->botonera_general('Transporte','total',$id); ?>
</div>
</form>
<!--
	Codigo
